...
void CLauncherDialog::OnBnClickedOk()
{
	...
	{
		if(!SharedUtility::ReadRegistryString(HKEY_CURRENT_USER, "Software\\NIV", "gtaivdir", NULL, szInstallDirectory, 
			sizeof(szInstallDirectory)))
		{
	...
	if(FindFirstFile(szApplicationPath, &fdFileInfo) == INVALID_HANDLE_VALUE)
	{
	...
	if(!CreateProcess(szApplicationPath, szParams, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL, SharedUtility::GetAppPath(), 
